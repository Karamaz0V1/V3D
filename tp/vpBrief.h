...
class vpBrief {
    public :
        vpBrief(int nb_pairs=VPBRIEF_NB_PAIRS, int patch_size=4); // Refactor nb_pairs = sizeof(type[n])
    ...
    private :
        ...
        void descriptorBitV2(const vpImage<unsigned char> & image, std::bitset<VPBRIEF_NB_PAIRS> & descriptor, std::vector<vpImagePoint>::const_iterator & it, int * pairs, int i);
